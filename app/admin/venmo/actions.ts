...
export async function sendVenmoReceipt(formData: FormData) {
  const supabase = await createClient();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) redirect("/admin/login");
  ...
  if (!process.env.RESEND_API_KEY) {
    redirect("/admin/venmo?error=RESEND_API_KEY%20is%20not%20configured");
  }
  ...
  if (order.status !== "approved") {
    redirect("/admin/venmo?error=Receipts%20can%20only%20be%20sent%20for%20approved%20orders");
  }
  if (!order.customer_email) {
    redirect("/admin/venmo?error=This%20order%20does%20not%20have%20a%20customer%20email");
  }
  ...
  revalidatePath("/admin/venmo");
  redirect("/admin/venmo?status=approved&saved=receipt");
}
